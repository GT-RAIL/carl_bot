...
carl_joy_teleop::carl_joy_teleop()
{
  ...
  scan_sub = node.subscribe<sensor_msgs::LaserScan>("scan", 10, &carl_joy_teleop::scan_cback, this);
  map_sub = node.subscribe<nav_msgs::OccupancyGrid>("/map", 10, &carl_joy_teleop::map_cback, this);
  amcl_pose_sub = node.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 10, &carl_joy_teleop::amcl_pose_cback, this);
  ...
  ROS_INFO("CARL Joystick Teleop Started (Brian)");

  puts(" ---------------------------------------------------");
  puts("| CARL Joystick Teleop Help  (Brian)                |");
  puts("|---------------------------------------------------|*");
...
void carl_joy_teleop::scan_cback(const sensor_msgs::LaserScan::ConstPtr& ptr)
{
  static int index = 0;
  ...
  if(closestDist < MIN_SAFE_DIST)
    forward_throttle_safety_factor_base = 0.0;
  else if(closestDist > START_SAFETY_THROTTLE_DIST)
    forward_throttle_safety_factor_base = 1.0;
  else{
    forward_throttle_safety_factor_base = (closestDist-MIN_SAFE_DIST)/(START_SAFETY_THROTTLE_DIST-MIN_SAFE_DIST);
    if(forward_throttle_safety_factor_base < 0.05) forward_throttle_safety_factor_base = 0.05;
  }
  
}

void carl_joy_teleop::map_cback(const nav_msgs::OccupancyGrid::ConstPtr& map){
  //captures the map for later use
  savedMap = *map;
}

void carl_joy_teleop::amcl_pose_cback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& pose){
  //get distances in occupancy grid units 
  static double startTrottleDistMap = (START_SAFETY_THROTTLE_DIST+0.15)/savedMap.info.resolution;
  static double minSafeDistMap = (MIN_SAFE_DIST+0.1)/savedMap.info.resolution;
